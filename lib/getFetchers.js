...
export const getUserCompanies = async (query, token) => {
  try {
    const result = await axios.get(`${base.apiUrl}/partners/users?${query}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    console.log(result.data);
    return { companies: result.data.data, pagination: result.data.pagination };
  } catch (error) {
    return { error };
  }
};

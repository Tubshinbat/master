...
import { getParticipation } from "lib/getFetchers";
...
export default function RootLayout({ children }) {
          ...
          <div className="action-btns">
            ...
            <Tooltip title="Устгах">
              <button
                ...
                onClick={async () => {
                  showModal("delete");
                  const { participation } = await getParticipation(record.key);
                  if (participation) {
                    setSelectData(participation);
                  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const partics = await axios.get(`participations?${query}`);
  ...
  const handleAdd = async (values) => {
    ...
    try {
      ...
      await axios.post("/participations", data);
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/participations/${selectData._id}`);
  ...
  const handleEdit = async (values) => {
    ...
    try {
      ...
      await axios.put(`/participations/${data.id}`, data);
  ...
  const showEdit = async (key) => {
    try {
      const { participation } = await getParticipation(key);
      if (participation) {
        form.setFieldsValue({ ...participation });
        if (participation.pictures && participation.pictures.length > 0) {
          setPictures(
            participation.pictures.map((img) => ({
              name: img,
          ...
          <div className="row">
            <div className="col-lg-12">
              <Form.Item
                ...
                rules={[requiredRule]}
              >
                <Input placeholder="Үйл ажиллагааны нэр оруулна уу" />
              </Form.Item>
            </div>

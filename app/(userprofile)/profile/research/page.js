...
import { getResearch } from "lib/getFetchers";
...
export default function RootLayout({ children }) {
  ...
  const [columns, setColumns] = useState([
...

    {
      dataIndex: "price",
      key: "price",
      title: "Судалгаанд төлсөн мөнгөн дүн",
      status: true,
          ...
          <div className="action-btns">
            ...
            <Tooltip title="Устгах">
              <button
                ...
                onClick={async () => {
                  showModal("delete");
                  const { research } = await getResearch(record.key);
                  if (research) {
                    setSelectData(research);
                  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const research = await axios.get(`researchs?${query}`);
      research && research.data && appendData(research.data.data);
      if (research.data && research.data.pagination) {
        const total = research.data.pagination.total;
        const pageSize = research.data.pagination.limit;
  ...
  const init = async () => {
    ...
    setLoading(true);
    const research = await axios.get(`/researchs?${query}`);
    if (research.data && research.data.data) appendData(research.data.data);
    setLoading(false);
  };
  ...
  const handleAdd = async (values) => {
    ...
    try {
      ...
      await axios.post("/researchs", data);
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/researchs/${selectData._id}`);
  ...
  const handleEdit = async (values) => {
    ...
    try {
      ...
      await axios.put(`/researchs/${data.id}`, data);
  ...
  const showEdit = async (key) => {
    try {
      const { research } = await getResearch(key);
      if (research) {
        form.setFieldsValue({ ...research });
        if (research.pictures && research.pictures.length > 0) {
          setPictures(
            research.pictures.map((img) => ({
              name: img,
        ...
        <Form layout="vertical" autoComplete="false" form={form}>
          <div className="row">
            <div className="col-lg-12">
              <Form.Item
                label="Судалгааны нэр"
                name="name"
                rules={[requiredRule]}
              >
                <Input placeholder="Судалгааны нэрийг оруулна уу" />
              </Form.Item>
            </div>
            ...
            <div className="col-lg-12">
              <Form.Item
                label="Судалгаанд зарцуулсан мөнгөн дүн"
                name="price"
                rules={[requiredRule]}
              >
                <InputNumber
                  placeholder="Судалгаанд зарцуулсан мөнгөн дүнгээ оруулна уу"
                  style={{ width: "100%" }}
                />
              </Form.Item>
            </div>
